...
export function ChargeTypeSection({ onChanged }: { onChanged?: () => void }) {
  ...
  async function save() {
    ...
    setSaving(true)
    const res = editing
      ? await apiClient.patch(`${API_BASE}/${editing._id}`, { name: name.trim(), applicableModules })
      : await apiClient.post(API_BASE, { name: name.trim(), applicableModules })
    if (res.success) {
  ...
  async function toggleModule(item: ChargeTypeItem, key: string) {
    ...
    setItems(prev => prev.map(i => (i._id === item._id ? { ...i, applicableModules } : i)))
    const res = await apiClient.patch(`${API_BASE}/${item._id}`, { applicableModules })
    if (!res.success) {
      toast.error(res.error)
  ...
  async function remove(id: string) {
    ...
    if (res.success) { toast.success('Charge type deleted'); load(); onChanged?.() }
    else toast.error(res.error)
  }

  const columns: ColumnDef<ChargeTypeItem>[] = [
    { key: 'name', header: 'Charge Type', accessor: 'name', sortable: true },
    ...MODULES.map((mod): ColumnDef<ChargeTypeItem> => ({
      key: mod.key,
      header: mod.label,
      align: 'center',
      render: item => (
      ...
      csvValue: item => (item.applicableModules.includes(mod.key) ? 'Yes' : 'No'),
    })),
    {
      key: 'active',
      header: 'Active',
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>This charge type will be removed.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
  ...
  return (
    <>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent showCloseButton={false} className="sm:max-w-none sm:w-[min(92vw,420px)] p-0 overflow-hidden gap-0">
