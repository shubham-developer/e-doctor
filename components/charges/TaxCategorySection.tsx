...
export function TaxCategorySection({ onChanged }: { onChanged?: () => void }) {
  ...
  async function save() {
    ...
    setSaving(true)
    const body = { name: name.trim(), percent: Number(percent) || 0, description: description.trim() }
    const res = editing
  ...
  async function remove(id: string) {
    ...
    if (res.success) { toast.success('Tax category deleted'); load(); onChanged?.() }
    else toast.error(res.error)
  }
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>This tax category will be removed.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent showCloseButton={false} className="sm:max-w-none sm:w-[min(92vw,420px)] p-0 overflow-hidden gap-0">
          <div className="bg-blue-600 text-white flex items-center justify-between px-5 py-3.5">
            <h2 className="text-base font-semibold">{editing ? 'Edit Tax Category' : 'Add Tax Category'}</h2>
