...
export function ChargesList({
  ...
  async function toggleActive(charge: Charge) {
    const res = await apiClient.patch(`/api/dashboard/charges/${charge._id}`, { isActive: !charge.isActive })
    if (res.success) load()
    else toast.error(res.error)
  }
  ...
  async function remove(id: string) {
    ...
    if (res.success) { toast.success('Charge deleted'); load() }
    else toast.error(res.error)
  }
  ...
  const columns: ColumnDef<Charge>[] = [
    ...
    },
    {
      ...
      header: 'Active',
      width: 'w-24',
      render: c => <Switch checked={c.isActive} onCheckedChange={() => toggleActive(c)} />,
    },
    {
      key: 'actions',
      header: '',
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This charge will be removed from the list. Existing bills won&apos;t be affected.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction className="bg-red-600 hover:bg-red-700" onClick={() => remove(c._id)}>
                  Delete
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
