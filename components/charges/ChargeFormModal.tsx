...
export function ChargeFormModal({
  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose() }}>
      <DialogContent showCloseButton={false} className="sm:max-w-none sm:w-[min(92vw,520px)] p-0 overflow-hidden gap-0">
        <div className="bg-blue-600 text-white flex items-center justify-between px-5 py-3.5">
          <h2 className="text-base font-semibold">{charge ? 'Edit Charge' : 'Add Charges'}</h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <SearchableSelect
                ...
                options={taxCategories.map(t => ({ value: t._id, label: t.name, sub: `${t.percent}%` }))}
