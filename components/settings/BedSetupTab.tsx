...
function RefList({ title, apiPath }: { title: string; apiPath: string }) {
  ...
  async function handleAdd() {
    ...
    const data = await res.json()
    if (data.success) { toast.success(`${title} added`); setNewName(''); load() }
            ...
            <tbody className="divide-y divide-gray-50">
              {items.map(item => (
                <tr key={item._id} className="hover:bg-gray-50 group">
                  <td className="px-4 py-2.5 text-xs text-gray-800">{item.name}</td>
...
function BedGroupSection() {
  ...
  useEffect(() => {
    fetch('/api/dashboard/floors').then(r => r.json()).then(d => {
  ...
  function openAdd() {
    setEditTarget(null); setFormName(''); setFormFloor(''); setFormDesc('')
    setDialogOpen(true)
  }
  ...
  async function handleDelete(id: string, name: string) {
    if (!confirm(`Delete "${name}"?`)) return
    const res  = await fetch(`/api/dashboard/bed-groups/${id}`, { method: 'DELETE' })
    const data = await res.json()
    if (data.success) { toast.success('Deleted'); load() }
    else toast.error(data.error)
  }
  ...
  const columns: ColumnDef<BedGroupRecord>[] = [
    {
      key: 'name', header: 'Name', sortable: true, sortValue: b => b.name,
      skeletonWidth: 'w-28',
      render: b => <span className="text-xs font-medium text-blue-600">{b.name}</span>,
    },
    {
      key: 'floor', header: 'Floor', sortable: true, sortValue: b => b.floor ?? '',
      width: 'w-32', skeletonWidth: 'w-20',
      render: b => <span className="text-xs text-gray-600">{b.floor || '—'}</span>,
    },
    {
    ...
    {
      key: 'actions', header: 'Action', align: 'right', width: 'w-20',
      skeletonWidth: 'w-12',
      render: b => (
        <div className="flex items-center justify-end gap-1">
          ...
          <button
            onClick={e => { e.stopPropagation(); handleDelete(b._id, b.name) }}
    ...
    <>
      <div className="flex flex-col h-full">
        ...
        <DataTable<BedGroupRecord>
          ...
          emptyNode={
            <div className="flex flex-col items-center gap-2 py-8">
              <p className="text-xs text-gray-400">No bed groups configured yet</p>
              <p className="text-xs text-gray-400">Click "+ Add Bed Group" to create one</p>
            </div>
          }
        ...
        <div className="flex items-center justify-between px-3 py-1.5 border-t border-gray-200 shrink-0 bg-gray-50 text-xs text-gray-500">
          <span>Records: {from} to {to} of {items.length}</span>
      ...
      <Dialog open={dialogOpen} onOpenChange={o => !o && setDialogOpen(false)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editTarget ? 'Edit Bed Group' : 'Add Bed Group'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-1">
            ...
            <div className="space-y-1.5">
              <Label>Floor</Label>
              <Select value={formFloor} onValueChange={v => setFormFloor(v ?? '')}>
                <SelectTrigger className="h-9">
                  <SelectValue placeholder="Select floor" />
                </SelectTrigger>
                <SelectContent>
                  {floors.length === 0
                    ? <SelectItem value="__none__" disabled>No floors — add in Floor section</SelectItem>
                    : floors.map(f => <SelectItem key={f._id} value={f.name}>{f.name}</SelectItem>)
                  }
                </SelectContent>
              </Select>
            </div>
...
function BedTable({ readOnly = false }: { readOnly?: boolean }) {
  ...
  function openAdd() {
    setEditTarget(null); setFormName(''); setFormBedType(''); setFormBedGroup(''); setFormUnavailable(false)
    setDialogOpen(true)
  }
  ...
  async function handleSave() {
    if (!formName.trim()) { toast.error('Bed name is required'); return }
    setSaving(true)
    try {
      const body = {
        name: formName.trim(),
  ...
  async function handleDelete(id: string, name: string) {
    ...
    const data = await res.json()
    if (data.success) { toast.success('Bed deleted'); loadBeds() }
    else toast.error(data.error)
  }
  ...
  const columns: ColumnDef<BedRecord>[] = [
    ...
    {
      key: 'bedType', header: 'Bed Type', sortable: true, sortValue: b => b.bedType,
      skeletonWidth: 'w-20',
      render: b => <span className="text-xs text-gray-600">{b.bedType || '—'}</span>,
    },
    {
      key: 'bedGroup', header: 'Bed Group', sortable: true, sortValue: b => b.bedGroup,
      skeletonWidth: 'w-28',
      render: b => <span className="text-xs text-gray-600">{b.bedGroup || '—'}</span>,
    },
    {
      key: 'status', header: 'Status', sortable: true, sortValue: b => b.status,
      skeletonWidth: 'w-20',
      render: b => (
        <span className={cn('text-xs font-medium', b.status === 'available' ? 'text-green-700' : 'text-red-600')}>
    ...
    ...(!readOnly ? [{
      ...
      render: (b: BedRecord) => (
        <div className="flex items-center justify-center gap-1">
          <button onClick={e => { e.stopPropagation(); openEdit(b) }} className="p-1.5 rounded hover:bg-blue-50 text-gray-400 hover:text-blue-600 transition-colors">
            <Pencil className="w-3.5 h-3.5" />
          </button>
          <button onClick={e => { e.stopPropagation(); handleDelete(b._id, b.name) }} className="p-1.5 rounded hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors">
            <Trash2 className="w-3.5 h-3.5" />
          </button>
        </div>
      ),
  ...
  return (
    <>
      <DataTable<BedRecord>
        ...
        loading={loading}
        skeletonRows={5}
        rowClassName={b => b.status === 'available' ? 'bg-green-50/60 hover:bg-green-50' : 'bg-red-50/50 hover:bg-red-50'}
        emptyNode={
          <div className="flex flex-col items-center gap-2 py-8">
            <p className="text-xs text-gray-400">No beds found</p>
            {!readOnly && <p className="text-xs text-gray-400">Click "+ Add Bed" to create one</p>}
          </div>
        }
        wrapperClassName="flex-1 overflow-auto"
        searchValue={search}
      ...
      {!readOnly && (
        <Dialog open={dialogOpen} onOpenChange={o => !o && setDialogOpen(false)}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-lg font-bold">{editTarget ? 'Edit Bed' : 'Add Bed'}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 pt-1">
              <div className="space-y-1.5">
                <Label>Name <span className="text-red-500">*</span></Label>
              ...
              <div className="space-y-1.5">
                <Label>Bed Type <span className="text-red-500">*</span></Label>
                <Select value={formBedType} onValueChange={v => setFormBedType(v ?? '')}>
                  <SelectTrigger className="h-10">
                    <SelectValue placeholder="Select" />
                  </SelectTrigger>
                  <SelectContent>
                    {bedTypes.length === 0
                      ? <SelectItem value="__none__" disabled>No bed types — add in Bed Type section</SelectItem>
                      : bedTypes.map(t => <SelectItem key={t._id} value={t.name}>{t.name}</SelectItem>)
                    }
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                <Label>Bed Group <span className="text-red-500">*</span></Label>
                <Select value={formBedGroup} onValueChange={v => setFormBedGroup(v ?? '')}>
                  <SelectTrigger className="h-10">
                    <SelectValue placeholder="Select" />
                  </SelectTrigger>
                  <SelectContent>
                    {bedGroups.length === 0
                      ? <SelectItem value="__none__" disabled>No bed groups — add in Bed Group section</SelectItem>
                      : bedGroups.map(g => <SelectItem key={g._id} value={g.name}>{g.name}</SelectItem>)
                    }
                  </SelectContent>
                </Select>
              </div>
              <label className="flex items-center gap-2.5 cursor-pointer select-none">
                ...
                />
                <span className="text-sm font-medium text-gray-700">Not available for use</span>
              </label>
            </div>
            <div className="flex gap-3 pt-2">
              ...
              <Button className="flex-1 bg-blue-600 hover:bg-blue-700 gap-2" onClick={handleSave} disabled={saving}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 shrink-0">
          <h2 className="text-sm font-semibold text-gray-800">
            {active === 'bed-group' ? 'Bed Group List' : SECTIONS.find(s => s.key === active)?.label}
          </h2>
        </div>
        ...
        <div className="flex-1 overflow-hidden">
          ...
          {active === 'bed'        && <BedTable />}
          {active === 'bed-type'   && <RefList title="Bed Type" apiPath="/api/dashboard/bed-types" />}
          {active === 'bed-group'  && <BedGroupSection />}
          {active === 'floor'      && <RefList title="Floor" apiPath="/api/dashboard/floors" />}
        </div>
      </div>
    </div>
  )
