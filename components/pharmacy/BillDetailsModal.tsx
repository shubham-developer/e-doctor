...
export function BillDetailsModal({
  ...
  return (
    <FormDialog
      open={!!bill}
      onClose={onClose}
      title={`Bill PHARMAB${bill?.billNumber}`}
      contentClassName="sm:w-[min(92vw,900px)]"
      footer={
        <>
          {balance > 0 && (
          ...
          <tbody>
            {(bill?.lines ?? []).map((ln, i) => (
              <tr key={i} className="border-b border-gray-100">
                ...
                <td className="py-1.5 pr-2 text-right">{fmt(ln.salePrice)}</td>
                <td className="py-1.5 pr-2 text-right">{ln.taxPercent}</td>
                <td className="py-1.5 pr-2 text-right">{ln.discountPercent}</td>
