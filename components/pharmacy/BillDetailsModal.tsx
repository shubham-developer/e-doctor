      ...
      >
        ...
        <div className="px-5 py-4 space-y-4 max-h-[75vh] overflow-y-auto">
          <div className="grid grid-cols-3 gap-3 text-sm">
            ...
            <div>
              <span className="block text-xs text-gray-500">Case ID</span>
            ...
            <div>
              <span className="block text-xs text-gray-500">Payment Mode</span>
            ...
            <thead>
              <tr className="border-b border-gray-200">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left py-2 pr-2 font-medium text-gray-600 whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
          ...
          <div className="flex justify-end">
            <div className="space-y-1 w-64">
              {[
                {
                  label: `Total (${symbol})`,
                  value: fmt(bill?.totalAmount ?? 0),
                },
                {
                  label: `Discount (${symbol})`,
                  value: fmt(bill?.discountAmount ?? 0),
                },
                { label: `Tax (${symbol})`, value: fmt(bill?.taxAmount ?? 0) },
                {
                  label: `Net Amount (${symbol})`,
                  value: fmt(bill?.netAmount ?? 0),
                },
                {
                  label: `Paid (${symbol})`,
                  value: fmt(bill?.paidAmount ?? 0),
                },
