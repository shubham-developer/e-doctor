...
function SortIcon({ active, dir }: { active: boolean; dir: "asc" | "desc" }) {
  if (!active)
    return (
      <ChevronsUpDown className="w-3.5 h-3.5 text-gray-300 group-hover:text-gray-500 transition-colors" />
    );
  return dir === "asc" ? (
...
export function DataTable<T extends object>({
  ...
  function downloadCSV() {
    ...
    const rows = displayData
      .map((row) =>
        exportCols
          .map((c) => `"${getCellText(c, row).replace(/"/g, '""')}"`)
          .join(","),
      )
      .join("\n");
