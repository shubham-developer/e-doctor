...
export async function getAgentReply(
  ...
  let patient = await Patient.findOne({ tenantId: tenant._id, whatsappNumber: patientPhone })
  if (!patient) {
  ...
  const slotList = slots
    .map((s) => `SlotID:${s._id} | ${s.doctorId.name} | ${s.startTime}-${s.endTime} | ₹${s.doctorId.consultationFee}`)
...
- If patient says "cancel" or "reschedule" help them do that
  ...
  // Save booking if Claude confirmed one
  await checkAndSaveBooking(reply, tenant._id.toString(), patient._id.toString(), slots)
...
async function checkAndSaveBooking(
  ...
  const existing = await Appointment.findOne({ bookingRef: bookingRefMatch[0] })
  if (existing) return
