...
export async function GET(req: NextRequest) {
  ...
  return apiResponse({ visits, total, page, limit, totalPages: Math.ceil(total / limit) })
}
...
export async function POST(req: NextRequest) {
  ...
  const {
    ...
    chiefComplaint, symptomsType, symptomsTitle, note, knownAllergiesOverride, previousMedicalIssue,
    caseNumber, reference, casualty, isOldPatient, isAntenatal, liveConsultation,
    charges, totalFee, appliedCharge, discount, tax, paymentMode, paidAmount, applyTpa,
  } = body
  ...
  const computedTotal = totalFee ?? chargeLines.reduce((s: number, c: { fee: number }) => s + (Number(c.fee) || 0), 0)
  ...
  const visit = await OpdVisit.create({
    ...
    ...(knownAllergiesOverride?.trim()   && { knownAllergiesOverride: knownAllergiesOverride.trim() }),
    ...
    ...(appliedCharge !== undefined && { appliedCharge: Number(appliedCharge) }),
  ...
  return apiResponse(
    {
      ...
      doctor: doctor ? { name: doctor.name, specialization: doctor.designation } : null,
    },
    201
  )
}
