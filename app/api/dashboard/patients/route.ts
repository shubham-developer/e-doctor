...
export async function GET(req: NextRequest) {
  ...
  return apiResponse({ patients, total, page, totalPages: Math.ceil(total / limit) })
}
...
export async function POST(req: NextRequest) {
  ...
  const { name, guardianName, gender, dateOfBirth, age, ageMonths, ageDays,
          bloodGroup, maritalStatus, phone, email, address, remarks, allergies,
          tpa, tpaId, tpaValidity, nationalId, alternateNumber, languagePref } = body
  ...
  try {
    ...
    const patient = await Patient.create({
      ...
      ...(alternateNumber?.trim() && { alternateNumber: alternateNumber.trim() }),
