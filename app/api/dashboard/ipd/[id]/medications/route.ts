...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const medications = await IpdMedication.find({ tenantId, ipdId: id }).sort({ createdAt: 1 })
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!unitPrice || Number(unitPrice) < 0) return apiError('Unit price is required', 400)
