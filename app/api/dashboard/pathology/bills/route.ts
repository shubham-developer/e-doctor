...
export async function POST(req: NextRequest) {
  ...
  const { patientId, caseId, billDate, referenceDoctor, previousReportValue,
          note, paymentMode, items, discount, paidAmount } = body

  if (!patientId) return apiError('Patient is required', 400)
  if (!items?.length) return apiError('At least one test is required', 400)

  // compute totals
  const amount    = (items as { amount: number }[]).reduce((s, i) => s + (i.amount ?? 0), 0)
  const taxTotal  = (items as { tax: number; charge: number }[]).reduce((s, i) => s + (i.charge ?? 0) * (i.tax ?? 0) / 100, 0)
  ...
  const last = await PathologyBill.findOne({ tenantId }).sort({ billNumber: -1 })
  const billNumber = (last?.billNumber ?? 0) + 1
  const billNo = `PATHOB${billNumber}`

  const bill = await PathologyBill.create({
    tenantId,
    billNo,
    billNumber,
    patientId,
