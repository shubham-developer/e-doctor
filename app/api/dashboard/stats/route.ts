...
export async function GET(req: NextRequest) {
  ...
  const radByMonth = Array<number>(12).fill(0);
  for (const r of opdMonthly)
    opdByMonth[r._id - 1] = Math.round(r.total * 100) / 100;
  for (const r of pharMonthly)
    pharByMonth[r._id - 1] = Math.round(r.total * 100) / 100;
  for (const r of pathMonthly)
    pathByMonth[r._id - 1] = Math.round(r.total * 100) / 100;
  for (const r of ipdMonthly)
    ipdByMonth[r._id - 1] = Math.round(r.total * 100) / 100;
  for (const r of radMonthly)
    radByMonth[r._id - 1] = Math.round(r.total * 100) / 100;
  ...
  const radTotal = radByMonth.reduce((a, b) => a + b, 0);
  ...
  return apiResponse({
    income: {
      ...
      radiology: Math.round(radTotal * 100) / 100,
      bloodBank: 0,
      ambulance: 0,
      general: 0,
    },
