...
export async function GET(req: NextRequest) {
  ...
  const [opdMonthly, pharMonthly, pathMonthly, ipdMonthly, radMonthly, totalPatients, totalStaff] = await Promise.all([
    OpdVisit.aggregate([
  ...
  for (const r of radMonthly)  radByMonth[r._id - 1]  = Math.round(r.total * 100) / 100

  const monthly = MONTHS.map((month, i) => ({
    month,
    income:   Math.round((opdByMonth[i] + pharByMonth[i] + pathByMonth[i] + ipdByMonth[i] + radByMonth[i]) * 100) / 100,
    expenses: 0,
  }))
