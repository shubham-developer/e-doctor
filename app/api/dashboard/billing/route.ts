...
async function patientFilter(tenantId: mongoose.Types.ObjectId, search: string) {
  if (!search) return null
  const pts = await Patient.find({ tenantId, name: { $regex: search, $options: 'i' } }, '_id').limit(200)
...
export async function GET(req: NextRequest) {
  ...
  if (module === 'summary') {
    ...
    const [opd, phar, path, rad, ipdAdm, ipdPay] = await Promise.all([
      OpdVisit.aggregate([
        ...
        { $group: { _id: null, count: { $sum: 1 }, collected: { $sum: '$paidAmount' } } },
      ]),
      PharmacyBill.aggregate([
        { $match: { tenantId: tid, createdAt: pharmDateRange } },
        { $group: { _id: null, count: { $sum: 1 }, net: { $sum: '$netAmount' }, paid: { $sum: '$paidAmount' } } },
      ]),
      PathologyBill.aggregate([
        { $match: { tenantId: tid, billDate: strRange(from, to) } },
        { $group: { _id: null, count: { $sum: 1 }, net: { $sum: '$netAmount' }, paid: { $sum: '$paidAmount' }, balance: { $sum: '$balance' } } },
      ]),
      RadiologyBill.aggregate([
        { $match: { tenantId: tid, billDate: strRange(from, to) } },
        { $group: { _id: null, count: { $sum: 1 }, net: { $sum: '$netAmount' }, paid: { $sum: '$paidAmount' }, balance: { $sum: '$balance' } } },
      ]),
      IpdAdmission.countDocuments({ tenantId: tid, admissionDate: strRange(from, to) }),
    ...
    return apiResponse({
      ...
      pharmacy:  { count: phar[0]?.count ?? 0, net: pharNet,  paid: pharPaid,  balance: Math.max(0, pharNet - pharPaid) },
      pathology: { count: path[0]?.count ?? 0, net: path[0]?.net ?? 0, paid: path[0]?.paid ?? 0, balance: path[0]?.balance ?? 0 },
      radiology: { count: rad[0]?.count  ?? 0, net: rad[0]?.net  ?? 0, paid: rad[0]?.paid  ?? 0, balance: rad[0]?.balance  ?? 0 },
  ...
  if (module === 'opd') {
    ...
    if (ptIds && ptIds.length === 0) return apiResponse({ bills: [], total: 0, totalPages: 0, page })

    const q: Record<string, unknown> = { tenantId: tid, visitDate: strRange(from, to) }
    if (ptIds) q.patientId = { $in: ptIds }

    const [total, bills] = await Promise.all([
      OpdVisit.countDocuments(q),
      OpdVisit.find(q)
        .populate('patientId', 'name patientCode age ageMonths ageDays gender bloodGroup address allergies previousMedicalIssue phone')
        .populate('doctorId',  'name specialization designation')
        .sort({ visitDate: -1, createdAt: -1 })
        .skip((page - 1) * limit)
        .limit(limit),
    ])
    return apiResponse({ bills, total, totalPages: Math.ceil(total / limit), page })
  }
  ...
  if (module === 'pharmacy') {
    ...
    const q: Record<string, unknown> = { tenantId: tid, createdAt: dateObjRange(from, to) }
    if (ptIds) q.patientId = { $in: ptIds }
    if (status === 'paid')    q.$expr = { $lte: ['$netAmount', '$paidAmount'] }
    if (status === 'partial') q.$expr = { $and: [{ $gt: ['$paidAmount', 0] }, { $gt: ['$netAmount', '$paidAmount'] }] }
  ...
  if (module === 'pathology') {
    ...
    const q: Record<string, unknown> = { tenantId: tid, billDate: strRange(from, to) }
    if (ptIds) q.patientId = { $in: ptIds }
    if (status === 'paid')    q.balance = 0
    if (status === 'partial') { q.balance = { $gt: 0 }; q.paidAmount = { $gt: 0 } }
    if (status === 'due')     q.paidAmount = { $lte: 0 }
  ...
  if (module === 'ipd') {
    ...
    const q: Record<string, unknown> = { tenantId: tid, admissionDate: strRange(from, to) }
    if (ptIds) q.patientId = { $in: ptIds }
    ...
    const bills = admissions.map(a => {
      ...
      return { ...a.toObject(), totalCharges, totalPaid, balance: totalCharges - totalPaid }
    })
