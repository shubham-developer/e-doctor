...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!tenantId) return apiError('Unauthorized', 401)
  if (userRole !== 'OWNER') return apiError('Only owners can update roles', 403)
  ...
  if (body.name !== undefined) {
    ...
    const conflict = await Role.findOne({ tenantId, name: body.name.trim(), _id: { $ne: id } })
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const userRole = req.headers.get('x-user-role')
  if (!tenantId) return apiError('Unauthorized', 401)
  if (userRole !== 'OWNER') return apiError('Only owners can delete roles', 403)
  ...
  await TenantUser.updateMany({ tenantId, customRoleId: id }, { $unset: { customRoleId: 1 } })
