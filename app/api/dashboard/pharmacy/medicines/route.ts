...
export async function GET(req: NextRequest) {
  ...
  const [medicines, total] = await Promise.all([
    Medicine.find(query).sort({ name: 1 }).skip((page - 1) * limit).limit(limit),
    Medicine.countDocuments(query),
  ])

  return apiResponse({ medicines, total, page, totalPages: Math.ceil(total / limit) })
}
...
export async function POST(req: NextRequest) {
  ...
  const {
    name, company, composition, category, group, unit,
    availableQty, salePrice, taxPercent, batchNo, expiryDate, reorderLevel,
  } = body
