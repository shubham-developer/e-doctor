...
function OpdForm({
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (chargeData.success) {
        const active: ChargeCategory[] = chargeData.data.filter((c: ChargeCategory) => c.isActive)
        setCategories(active)
        // Pre-select all active charges with their default fees
        setSelectedCharges(active.map((c) => ({ categoryId: c._id, name: c.name, fee: String(c.defaultFee) })))
      }
    })
  }, [])
  ...
  function toggleCharge(cat: ChargeCategory) {
    setSelectedCharges((prev) => {
      ...
      return [...prev, { categoryId: cat._id, name: cat.name, fee: String(cat.defaultFee) }]
    })
  }

  function updateFee(categoryId: string, fee: string) {
    setSelectedCharges((prev) => prev.map((c) => c.categoryId === categoryId ? { ...c, fee } : c))
  }

  const total = selectedCharges.reduce((sum, c) => sum + (Number(c.fee) || 0), 0)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const chargePayload = selectedCharges.map((c) => ({ categoryId: c.categoryId, name: c.name, fee: Number(c.fee) || 0 }))
      ...
      toast.success(t('generated', { number: String(opdNumber).padStart(3, '0') }))
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-lg px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          {patient.age > 0 && <p className="text-xs text-gray-500">{patient.age} yrs</p>}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea id="complaint" value={chiefComplaint} onChange={(e) => setChiefComplaint(e.target.value)} placeholder={t('complaintPlaceholder')} rows={2} />
      </div>
                ...
                <div key={cat._id} className={`flex items-center gap-3 px-3 py-2.5 transition-colors ${selected ? 'bg-blue-50/60' : 'bg-white'}`}>
                  ...
                  <div className="relative w-24">
                    ...
                    <Input
                      ...
                      onChange={(e) => selected && updateFee(cat._id, e.target.value)}
                      disabled={!selected}
                      min={0}
...
export default function PatientsPage() {
  ...
  const loadPatients = useCallback(async (p = page, q = search, size = pageSize) => {
    setLoading(true)
  ...
  useEffect(() => {
    const id = setTimeout(() => { setPage(1); loadPatients(1, search, pageSize) }, 300)
    return () => clearTimeout(id)
  }, [search])

  function handlePageChange(next: number) { setPage(next); loadPatients(next, search, pageSize) }
  function handlePageSizeChange(val: string) {
    const size = Number(val); setPageSize(size); setPage(1); loadPatients(1, search, size)
  }

  async function handleAdd(body: PatientFormData) {
    const res = await fetch('/api/dashboard/patients', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    const data = await res.json()
    if (data.success) { toast.success(t('addedSuccess')); setPage(1); loadPatients(1, search, pageSize) }
    else { toast.error(data.error); throw new Error(data.error) }
  }

  async function handleEdit(body: PatientFormData) {
    if (!editPatient) return
    const res = await fetch(`/api/dashboard/patients/${editPatient._id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    const data = await res.json()
    if (data.success) { toast.success(t('updatedSuccess')); loadPatients(page, search, pageSize) }
    else { toast.error(data.error); throw new Error(data.error) }
  }

  async function handleDelete(patient: Patient) {
    if (!confirm(`Delete "${patient.name}"? This cannot be undone.`)) return
    const res = await fetch(`/api/dashboard/patients/${patient._id}`, { method: 'DELETE' })
    const data = await res.json()
    if (data.success) { toast.success('Patient deleted'); loadPatients(page, search, pageSize) }
    else toast.error(data.error)
  }

  async function handleBulkDelete() {
    if (!confirm(`Delete ${selectedIds.size} selected patient(s)? This cannot be undone.`)) return
    await Promise.all([...selectedIds].map(id => fetch(`/api/dashboard/patients/${id}`, { method: 'DELETE' })))
    toast.success(`${selectedIds.size} patients deleted`)
    setSelectedIds(new Set())
    setPage(1); loadPatients(1, search, pageSize)
  }
          ...
          {canEdit && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-44">
                ...
                <DropdownMenuItem onClick={() => setEditPatient(r)} className="gap-2 text-sm cursor-pointer">
                  <Info className="w-3.5 h-3.5" /> Edit Patient
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => handleDelete(r)} className="gap-2 text-sm text-red-600 focus:text-red-600 cursor-pointer">
    ...
    <div className="space-y-0 h-full flex flex-col">
      ...
      <DataTable
        ...
        onSelectRow={(key, checked) =>
          setSelectedIds(prev => { const n = new Set(prev); checked ? n.add(key) : n.delete(key); return n })
        }
        ...
        toolbarRight={
          <>
            ...
            <Select value={String(pageSize)} onValueChange={v => handlePageSizeChange(v ?? '')}>
      ...
      <div className="flex items-center justify-between pt-3 px-1 text-xs text-gray-500">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="px-2 text-xs font-medium">{t('pageOf', { page, totalPages })}</span>
          <Button variant="outline" size="sm" className="h-7 w-7 p-0" disabled={page >= totalPages} onClick={() => handlePageChange(page + 1)}>
      ...
      <Dialog open={!!editPatient} onOpenChange={(o) => !o && setEditPatient(null)}>
        <DialogContent className="w-[95vw] sm:max-w-3xl">
          ...
          {editPatient && <PatientForm initial={editPatient} onSave={handleEdit} onClose={() => setEditPatient(null)} />}
        </DialogContent>
      </Dialog>

      <Dialog open={!!opdPatient} onOpenChange={(o) => !o && setOpdPatient(null)}>
