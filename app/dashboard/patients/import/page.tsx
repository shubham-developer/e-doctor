...
import { ChevronLeft, Download, Upload, FileText, X, CheckCircle, AlertCircle } from 'lucide-react'
...
function downloadSample() {
  ...
  const sample  = 'Ramesh Kumar,Suresh Kumar,Male,35,6,10,Married,9876543210,ramesh@example.com,"123 Main St Jhansi",,Penicillin,ABCD1234,,2026-12-31,B+'
  const blob = new Blob([`${headers}\n${sample}`], { type: 'text/csv;charset=utf-8;' })
  const url  = URL.createObjectURL(blob)
  const a    = document.createElement('a'); a.href = url; a.download = 'sample_patients.csv'; a.click()
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-1">
          {INSTRUCTIONS.map((text, i) => (
            <p key={i} className="text-sm text-gray-600">{i + 1}. {text}</p>
          ))}
        </div>
        ...
        <div className="overflow-x-auto border-t border-gray-100">
          <table className="w-full text-sm">
            ...
            <tbody>
              <tr>
                {COLUMNS.map(col => (
                  <td key={col.key} className="px-3 py-2 text-gray-400 border-r border-gray-100 last:border-r-0 whitespace-nowrap">
                    Sample Data
                  </td>
                ))}
              </tr>
            ...
            >
              {file ? (
                ...
                <span className="text-gray-400 flex items-center gap-1.5">
                  <Upload className="w-4 h-4" /> Drop a file here or click to browse
                </span>
              )}
            </div>
      ...
      {rows.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
          ...
          {/* Preview table */}
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Preview (first 5 valid rows)</p>
            <div className="overflow-x-auto rounded-md border border-gray-200">
              <table className="w-full text-xs">
                <thead className="bg-gray-50">
                  <tr>
                    {COLUMNS.slice(0, 8).map(col => (
                      <th key={col.key} className="px-3 py-2 text-left font-semibold text-gray-600 whitespace-nowrap border-b border-gray-200">{col.csv}</th>
                    ))}
                ...
                <tbody>
                  {validRows.slice(0, 5).map((row, i) => (
                    <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                      {COLUMNS.slice(0, 8).map(col => (
                        <td key={col.key} className="px-3 py-1.5 text-gray-700 border-b border-gray-100 whitespace-nowrap max-w-32 truncate">
                          {row[col.key] || <span className="text-gray-300">—</span>}
                        </td>
                      ))}
                      <td className="px-3 py-1.5 text-gray-400 border-b border-gray-100">…</td>
                    </tr>
                  ))}
                </tbody>
              </table>
