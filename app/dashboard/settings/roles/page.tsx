...
export default function RolesPage() {
  ...
  async function deleteRole(role: Role) {
    ...
    const res = await fetch(`/api/dashboard/settings/roles/${role._id}`, { method: 'DELETE' })
    const data = await res.json()
    if (data.success) {
              ...
              {renaming ? (
                ...
                <>
                  ...
                  {isOwner && !selected.isSystem && (
                    <button
                      onClick={() => { setNewName(selected.name); setRenaming(true) }}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 w-full">Module</th>
                  <th className="px-4 py-2.5 text-xs font-semibold text-gray-500 text-center w-16">All</th>
                  {COLS.map(col => (
                    <th key={col} className="px-4 py-2.5 text-xs font-semibold text-gray-500 text-center w-16 capitalize">
                      {col}
                    </th>
                  ))}
                </tr>
                ...
                {MODULES.map(mod => {
                  ...
                  return (
                    <tr key={mod.key} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-4 py-3 text-sm text-gray-700 font-medium">{mod.label}</td>
