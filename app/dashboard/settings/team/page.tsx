...
export default function TeamPage() {
  ...
  async function addUser() {
    ...
    if (data.success) {
      toast.success(`User added! Temp password: ${data.data.tempPassword}`, {
        duration: 10000,
      });
      setNewUserName("");
      setNewUserEmail("");
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {customRoles.length > 0 && (
                <Select
                  value={newCustomRole}
                  onValueChange={(v) =>
                    setNewCustomRole(!v || v === "__none__" ? "" : v)
                  }
                >
                  <SelectTrigger className="h-10">
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader>
          <CardTitle className="text-base">
            {t("teamMembersTitle")} ({users.length})
          </CardTitle>
        </CardHeader>
              ...
              <div key={u._id} className="flex items-center gap-3 p-4">
                ...
                {isOwner && u._id !== user?.id ? (
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    ...
                    <Select
                      value={u.role}
                      onValueChange={(r) => r && changeRole(u._id, r)}
                    >
                  ...
                  <div className="flex items-center gap-1.5">
                    <Badge
                      className={`${ROLE_BADGE[u.role] ?? ROLE_BADGE.VIEWER} border-0`}
                    >
                      {u.role}
                    </Badge>
                    {u.customRoleId && (
