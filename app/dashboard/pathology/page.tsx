...
import { Plus, X, FlaskConical, Search, Printer, CheckCircle } from 'lucide-react'
...
// ── Generate Bill Dialog (full-screen) ────────────────────────────────────────
...
function GenerateBillDialog({
  ...
  useEffect(() => {
    fetch('/api/dashboard/pathology/tests')
      .then(r => r.json())
  ...
  async function handleSave(print = false) {
    ...
    setSubmitting(true)
    try {
      const res  = await fetch('/api/dashboard/pathology/bills', {
        method: 'POST',
      ...
      const bill: PathologyBill = data.data
      onSaved(bill)
      if (print) {
...
export default function PathologyPage() {
  ...
  function handlePrint(b: PathologyBill) {
  ...
  const load = useCallback(async (p = page) => {
    setLoading(true)
    const res  = await fetch(`/api/dashboard/pathology/bills?page=${p}&limit=25${search ? `&search=${encodeURIComponent(search)}` : ''}`)
    const data = await res.json()
    if (data.success) {
  ...
  const columns: ColumnDef<PathologyBill>[] = [
    ...
    <>
      ...
      <div className="h-full flex flex-col bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-200 shrink-0 bg-gray-50 px-3 py-2">
          <h1 className="text-sm font-semibold text-gray-800">Pathology Bills</h1>
          <div className="flex items-center gap-2">
            <Button
              size="sm"
              variant="outline"
              className="h-8 text-xs gap-1"
              onClick={() => router.push('/dashboard/pathology/tests')}
            >
              <FlaskConical className="w-3.5 h-3.5" /> Pathology Test
            </Button>
            {canEdit && (
        ...
        <DataTable<PathologyBill>
          ...
          downloadable
          printable
          fileName="pathology-bills"
          emptyText="No pathology bills found. Click '+ Generate Bill' to create one."
        />
