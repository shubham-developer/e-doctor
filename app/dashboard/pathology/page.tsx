...
export default function PathologyPage() {
  ...
  const {
    ...
    isPending: loading,
    refetch: load,
  } = useApiQuery<{ bills: PathologyBill[]; total: number }>(
    ["pathology-bills", search, page],
    `/api/dashboard/pathology/bills?page=${page}&limit=25${search ? `&search=${encodeURIComponent(search)}` : ""}`,
    { keepPrevious: true },
  );
