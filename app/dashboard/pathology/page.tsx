...
import { Plus, FlaskConical, Printer } from 'lucide-react'
...
export default function PathologyPage() {
  ...
  const load = useCallback(async (p = page) => {
    setLoading(true)
    const res = await apiClient.get<{ bills: PathologyBill[]; total: number }>(`/api/dashboard/pathology/bills?page=${p}&limit=25${search ? `&search=${encodeURIComponent(search)}` : ''}`)
    if (res.success) {
    ...
    <>
      ...
      <div className="h-full flex flex-col bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-200 shrink-0 bg-gray-50 px-3 py-2">
          <h1 className="text-lg font-semibold text-gray-800">Pathology Bills</h1>
