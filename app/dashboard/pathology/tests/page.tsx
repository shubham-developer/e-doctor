...
function TestDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true)
    try {
      const url    = test ? `/api/dashboard/pathology/tests/${test._id}` : '/api/dashboard/pathology/tests'
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl flex flex-col" style={{ maxHeight: '90vh' }}>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className={lbl}>Report Days <span className="text-red-500">*</span></label>
...
export default function PathologyTestsPage() {
  ...
  const [editTest, setEditTest]   = useState<PathologyTest | null>(null)
  const canEdit = user?.role !== 'VIEWER'

  const load = useCallback(async () => {
    setLoading(true)
    const res = await fetch('/api/dashboard/pathology/tests').then(r => r.json())
  ...
  async function handleDelete(t: PathologyTest) {
    if (!confirm(`Delete "${t.name}"?`)) return
    const res  = await fetch(`/api/dashboard/pathology/tests/${t._id}`, { method: 'DELETE' })
    const data = await res.json()
    ...
    <>
      ...
      <div className="h-full flex flex-col bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-200 shrink-0 bg-gray-50 px-3 py-2">
          <h1 className="text-sm font-semibold text-gray-800">Pathology Test</h1>
          {canEdit && (
            <Button size="sm" className="h-8 text-xs gap-1 bg-blue-600 hover:bg-blue-700" onClick={() => setShowAdd(true)}>
              <Plus className="w-3.5 h-3.5" /> Add Pathology Test
            </Button>
          )}
        </div>

        <DataTable<PathologyTest>
          columns={columns}
          data={filtered}
          rowKey={t => t._id}
          loading={loading}
