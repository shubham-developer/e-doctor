...
const STATUS_CFG: Record<AttStatus, { label: string; bg: string; text: string; dot: string }> = {
  ...
  holiday:  { label: "Holiday",  bg: "bg-purple-100", text: "text-purple-700", dot: "bg-purple-500" },
};
...
function TodayAttendancePanel() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const res = await apiClient.get<{ staff: StaffWithStatus[]; summary: TodaySummary }>(
        ...
        <div className="flex items-center gap-2.5">
          ...
          {summary && (
            <div className="flex items-center gap-2 ml-2">
              {summary.present > 0    && <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-2xs font-semibold">{summary.present} Present</span>}
              {summary.absent > 0     && <span className="px-2 py-0.5 bg-red-100 text-red-700 rounded-full text-2xs font-semibold">{summary.absent} Absent</span>}
              {summary.onLeave > 0    && <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-2xs font-semibold">{summary.onLeave} On Leave</span>}
              {summary.halfDay > 0    && <span className="px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full text-2xs font-semibold">{summary.halfDay} Half Day</span>}
              {summary.notMarked > 0  && <span className="px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full text-2xs font-semibold">{summary.notMarked} Not Marked</span>}
            </div>
          )}
        </div>
      ...
      {open && (
        <div className="px-6 pb-4 space-y-3">
          ...
          {loading ? (
            ...
            <p className="text-xs text-gray-400 py-4 text-center">No staff in this category</p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 max-h-48 overflow-y-auto pr-1">
              {filtered.map((s) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-bold text-white ${cfg ? cfg.dot : "bg-gray-300"}`}>
                      {s.name.charAt(0).toUpperCase()}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-gray-800 truncate">{s.name}</p>
                      {cfg ? (
                        <span className={`text-2xs font-semibold ${cfg.text}`}>{cfg.label}</span>
                      ) : (
                        <span className="text-2xs text-gray-400">Not marked</span>
                      )}
                    </div>
                  </div>
                );
