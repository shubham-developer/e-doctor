  ...
  function searchPatients(q: string) {
    ...
    searchRef.current = setTimeout(async () => {
      const res = await fetch(
        `/api/dashboard/patients?search=${encodeURIComponent(q)}&limit=10`,
      );
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-4 pt-1">
            ...
            <div className="relative">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onFocus={() => {
                    if (!selectedPat) setDropOpen(true);
                  }}
          ...
          <div>
            ...
            >
              <ChevronDown
                className={`w-3.5 h-3.5 transition-transform ${showVitals ? "rotate-180" : ""}`}
              />
            ...
            {showVitals && (
              <div className="mt-3 grid grid-cols-3 gap-2">
                {(
                  ...
                  <div key={f.key}>
                    <label className="block text-2xs text-gray-500 mb-1">
                      {f.label}{" "}
                      {f.unit && (
...
function NoteCard({
  ...
  const displayNote =
    isLong && !expanded ? note.note.slice(0, 180) + "…" : note.note;
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 hover:border-primary-200 transition-colors">
      ...
      {hasVitals(note.vitalSigns) && (
        <div className="mt-2.5 flex flex-wrap gap-1.5">
          ...
          <VitalChip
            label="Pulse"
            value={note.vitalSigns?.pulse?.toString()}
            unit=" bpm"
          />
          <VitalChip
            label="Temp"
            value={note.vitalSigns?.temp?.toString()}
            unit="°F"
          />
          <VitalChip
            label="Wt"
            value={note.vitalSigns?.weight?.toString()}
            unit=" kg"
          />
          <VitalChip
            label="O₂"
            value={note.vitalSigns?.o2Sat?.toString()}
            unit="%"
          />
          <VitalChip
            label="RR"
            value={note.vitalSigns?.respRate?.toString()}
            unit="/min"
          />
        </div>
      )}
...
export default function NurseNotesPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res = await fetch(`/api/dashboard/nurse-notes/${id}`, {
      method: "DELETE",
    });
    const data = await res.json();
    if (data.success) {
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            {!search && canWrite && (
              <p className="text-sm mt-1">
                Click "Add Note" to record the first observation
              </p>
            )}
          </div>
        ) : (
          filtered.map((n) => (
            <NoteCard
              ...
              canDelete={
                canWrite &&
                (n.addedByName === user?.name || user?.role === "OWNER")
              }
